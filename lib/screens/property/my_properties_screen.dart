...
import 'package:flutter_modular/flutter_modular.dart'; // <-- Importación corregida a Modular
...
class _MyPropertiesScreenState extends State<MyPropertiesScreen> {
  ...
  Future<List<Property>> _fetchData() {
    // Usamos Provider.of con listen: false para que este método no fuerce un rebuild innecesario
    return Modular.get<MobiliariaProvider>().fetchUserProperties();
  }

  // Navega a la pantalla de edición, pasando el objeto Property.
  void _editProperty(Property property) {
    // Modular usa pushNamed con 'arguments' para pasar objetos
    // La ruta es '/property/new' o '/property/edit' si existiera, pero usamos '/new' para la misma pantalla.
    ...
    if (mounted) {
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              Modular.get<MobiliariaProvider>().errorMessage ??
                  'Error al eliminar.',
            ),
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 20),
                  ElevatedButton.icon(
                    // Modular: pushNamed para la ruta '/property/new'
  ...
  Widget _buildPropertyItem(Property property, BuildContext context) {
    return Card(
      ...
      child: InkWell(
        // Modular: pushNamed para navegar al detalle (la ruta completa es /property/detail/:id)
